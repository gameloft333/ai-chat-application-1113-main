-- Supabase AI Chat Application Initial Schema Migration
-- Version: 1.5 (Add character_id column safely if messages table pre-exists)
-- Date: [Current Date]

-- =====================================================
-- Setup Extensions (Ensure required extensions are enabled)
-- =====================================================
-- Enable pg_tle for timestamp functions if not already enabled
-- CREATE EXTENSION IF NOT EXISTS pg_tle; -- Uncomment if needed and have permissions
...
-- 4. Messages Table - CREATE WITHOUT character_id first to handle existing tables case
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Table doesn't exist, create it with character_id included
  ...
  ELSE
    -- Table exists, make sure it has a character_id column
    IF NOT EXISTS (
      SELECT FROM information_schema.columns 
      WHERE table_schema = 'public' AND table_name = 'messages' AND column_name = 'character_id'
    ) THEN
      -- Add character_id column to existing table
      ALTER TABLE public.messages ADD COLUMN character_id UUID NULL REFERENCES public.characters(id) ON DELETE SET NULL;
      COMMENT ON COLUMN public.messages.character_id IS 'Character ID if the role is assistant. Added by migration v1.5.';
    END IF;
  END IF;
END
$$;
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Table exists, make sure it has a transaction_id column
    IF NOT EXISTS (
      SELECT FROM information_schema.columns 
      WHERE table_schema = 'public' AND table_name = 'payments' AND column_name = 'transaction_id'
    ) THEN
      -- Add transaction_id column to existing table
      ALTER TABLE public.payments ADD COLUMN transaction_id TEXT NULL UNIQUE;
      COMMENT ON COLUMN public.payments.transaction_id IS 'Unique transaction identifier from the payment gateway. Added by migration.';
    END IF;
  ELSE
    -- Table doesn't exist, create it with transaction_id included
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Table exists, make sure it has provider_subscription_id column
    IF NOT EXISTS (
      SELECT FROM information_schema.columns 
      WHERE table_schema = 'public' AND table_name = 'subscriptions' AND column_name = 'provider_subscription_id'
    ) THEN
      -- Add provider_subscription_id column to existing table
      ALTER TABLE public.subscriptions ADD COLUMN provider_subscription_id TEXT NULL UNIQUE;
      COMMENT ON COLUMN public.subscriptions.provider_subscription_id IS 'Unique subscription identifier from the payment gateway. Added by migration.';
    END IF;
  ELSE
...
-- Indexes for messages - First verify character_id column exists before creating the index
DO $$
BEGIN
  -- Check if the character_id column exists in messages table
  IF EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'messages' AND column_name = 'character_id'
  ) THEN
    ...
    IF NOT EXISTS (
      ...
      WHERE schemaname = 'public' AND tablename = 'messages' AND indexname = 'idx_messages_character_id'
    ) THEN
      CREATE INDEX idx_messages_character_id ON public.messages(character_id);
    END IF;
  END IF;
END
$$;

-- Other message indexes that don't depend on character_id
...
-- Check if the transaction_id column exists in payments table before creating the index
DO $$
BEGIN
  -- Check if the transaction_id column exists in payments table
  IF EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'payments' AND column_name = 'transaction_id'
  ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_indexes 
      WHERE schemaname = 'public' AND tablename = 'payments' AND indexname = 'idx_payments_transaction_id'
    ) THEN
...
-- Check if the provider_subscription_id column exists in subscriptions table before creating the index
DO $$
BEGIN
  -- Check if the provider_subscription_id column exists in subscriptions table
  IF EXISTS (
    SELECT FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'subscriptions' AND column_name = 'provider_subscription_id'
  ) THEN
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_indexes 
      WHERE schemaname = 'public' AND tablename = 'subscriptions' AND indexname = 'idx_subscriptions_provider_subscription_id'
    ) THEN
...
-- Trigger to sync on new user creation in auth.users
-- Function Definition (depends on users table existing)
...
BEGIN
  ...
  ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email, -- Update email if it changes in auth.users
    updated_at = timezone('utc'::text, now()); -- Ensure updated_at reflects sync
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- =====================================================
-- Helper Functions (Dependent Functions - AFTER Tables and Triggers)
-- =====================================================
...
-- Update the default character ID in system settings
UPDATE public.system_settings
SET value = '"00000000-0000-0000-0000-000000000001"'
WHERE key = 'default_character_id'
AND value::text = 'null';
...
-- For the messages table, add comment if it's missing
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE n.nspname = 'public' AND c.relname = 'messages' AND d.objsubid = 0
  ) THEN
    COMMENT ON TABLE public.messages IS 'Stores individual messages within a conversation.';
  END IF;
END
...
-- For the payments table, add comment if it's missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_description d 
    JOIN pg_class c ON d.objoid = c.oid 
    JOIN pg_namespace n ON c.relnamespace = n.oid
    WHERE n.nspname = 'public' AND c.relname = 'payments' AND d.objsubid = 0
  ) THEN
    COMMENT ON TABLE public.payments IS 'Records payment transactions for subscriptions or one-time purchases.';
  END IF;
END
...
-- End of Migration
-- =====================================================
COMMENT ON DATABASE postgres IS 'Initial schema setup for AI Chat Application completed (v1.6 - Added sample data and fixed missing descriptions).'; 