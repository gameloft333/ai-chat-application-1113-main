...
check_disk_space() {
    ...
    echo -e "${YELLOW}当前可用空间: ${available_space}GB${NC}"
    echo -e "${YELLOW}所需最小空间: ${required_space}GB${NC}"
    
    if [ "${available_space%.*}" -lt "$required_space" ]; then
        echo -e "${RED}[ERROR] 磁盘空间不足！需要至少 ${required_space}GB${NC}"
        
        # 显示 Docker 资源使用情况
        echo -e "${YELLOW}当前 Docker 资源使用情况：${NC}"
        docker system df
        
        echo -e "${YELLOW}[CLEAN] 开始清理 Docker 资源...${NC}"
        
        # 分步清理，更安全
        echo -e "${YELLOW}1. 清理未使用的镜像${NC}"
        docker image prune -f
        
        echo -e "${YELLOW}2. 清理构建缓存${NC}"
        docker builder prune -f
        
        echo -e "${YELLOW}3. 清理已停止的容器${NC}"
        docker container prune -f
        
        echo -e "${YELLOW}4. 清理未使用的网络${NC}"
        docker network prune -f
        
        # 再次检查空间
        available_space=$(df -BG / | awk 'NR==2 {print $4}' | sed 's/G//')
        echo -e "${YELLOW}清理后可用空间: ${available_space}GB${NC}"
        
        if [ "${available_space%.*}" -lt "$required_space" ]; then
            echo -e "${RED}[ERROR] 清理后仍然空间不足${NC}"
            ...
            echo -e "${YELLOW}继续部署，但可能会遇到问题${NC}"
        else
            echo -e "${GREEN}空间清理完成，可以继续部署${NC}"
        fi
    else
        echo -e "${GREEN}[OK] 磁盘空间充足${NC}"
    fi
    
    return 0
}

# 在 check_and_install_docker 函数后添加
check_and_clean_disk_space() {
    ...
    if [ "${available_space%.*}" -lt "$required_space" ]; then
        echo -e "${YELLOW}磁盘空间不足，开始清理...${NC}"
        
        echo -e "${YELLOW}1. 查看当前 Docker 资源使用情况${NC}"
        docker system df
        
        echo -e "${YELLOW}2. 清理未使用的镜像${NC}"
        docker image prune -f
        
        echo -e "${YELLOW}3. 清理构建缓存${NC}"
        docker builder prune -f
        
        echo -e "${YELLOW}4. 清理已停止的容器${NC}"
        docker container prune -f
        
        echo -e "${YELLOW}5. 清理未使用的网络${NC}"
        docker network prune -f
        ...
        echo -e "${GREEN}磁盘空间充足，可以继续部署${NC}"
    fi
    
    return 0
}

# 修改 pre_deployment_check 函数，添加磁盘空间检查
